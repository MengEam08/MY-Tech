...
const Edit = ({ placeholder }) => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          reset({
            ...
            qty: result.data.qty,
            status: result.data.status === 1 ? "yes" : "no", // Ensure correct conversion
            is_featured: result.data.is_featured === "yes" ? 1 : 0,
          });
        } else {
          toast.error(result.message); // Display error message from the backend
        }
      } catch (error) {
  ...
  const saveProduct = async (data) => {
    ...
    formData.append("status", data.status === "yes" ? 1 : 0);
    formData.append("is_featured", data.is_featured == 1 ? "yes" : "no");
